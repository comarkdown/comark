...
Equivalence is structural for primitive options and by identity for `plugins`, `autoClose` and `tracer`. Create plugin instances once rather than inline on every render, or every call gets its own parser:
...
Creates a reusable parser function with pre-configured options. Unlike `getMarkdownParser()`, this always builds a fresh parser, so use it when you need one nobody else holds, like a streaming consumer.
