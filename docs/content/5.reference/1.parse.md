...
| `headingIds` | `boolean` | `true` | Auto-generate `id` attributes for `h1`–`h6` from visible heading text (not tag or component names). An explicit `id` attribute wins. Set `false` to disable. See [Headings](/syntax/markdown#headings). |
