...
description: Map every MDX concept (components, props, expressions, imports and exports) to its Comark equivalent, with a step-by-step migration checklist.
navigation:
...
<template>
  <Suspense>
    <Markdown :components="{ Alert, Chart }">{{ markdown }}</Markdown>
  </Suspense>
</template>
```
...
export default function Page() {
  return <Markdown components={{ Alert, Chart }}>{markdown}</Markdown>
}
```
::

For shared defaults, use `defineMarkdownComponent` to avoid passing components everywhere:

```typescript
import { defineMarkdownComponent } from '@comark/vue' // or '@comark/react'
import Alert from './components/Alert'
import Chart from './components/Chart'
