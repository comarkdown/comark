...
| **Panel** | `comark/devtools/renderer` | Internal — UI panel rendered inside the Vite DevTools iframe (not intended for end users) |
...
When `comarkDevtools()` is in your Vite config, it injects a client entry that connects `import.meta.hot` to the ambient context — listing keys, reverse-rendering markdown for the panel, and applying panel edits via `doc.set`.
