...
::tip{to="/syntax/markdown-ast"}
See the Markdown AST reference for the full node type documentation and traversal helpers.
::
...
`@comark/nuxt` auto-imports `Markdown`, `MarkdownParsed`, `defineMarkdownComponent`, and `defineMarkdownParsedComponent`. Plugin and component configuration moves out of `nuxt.config.ts` and into dedicated component definitions.
...
```
```vue [After]
<MarkdownParsed :value="tree" :components="components" />
```
::
...
|---|---|---|
| `body` (`MDCRoot`) | `value` (`MarkdownTree`) | Different shape, see AST section |
...
| `unwrap` | `unwrap` prop / parse option | `<Markdown unwrap />` or `:options="{ unwrap: 'p' }"` — see [inline rendering](/api/parse#inline-rendering) |
...
For a pre-parsed tree, use `<MarkdownParsed>` directly instead of passing it to the all-in-one component.
...
In MDC, plugins were configured once in `nuxt.config.ts` and applied globally to every parse call. In Comark, you define one or more named components using `defineMarkdownComponent` and `defineMarkdownParsedComponent`, each with its own plugins. This means different parts of your app can use different configurations without any workarounds.
...
import { defineMarkdownComponent, defineMarkdownParsedComponent } from '@comark/vue'
import highlight from 'comark/plugins/highlight'
import toc from 'comark/plugins/toc'
import emoji from 'comark/plugins/emoji'
...
The same pattern applies to `defineMarkdownParsedComponent` when you pre-parse on the server:
...
| Reusable configured renderer | — | `defineMarkdownParsedComponent({ components })` |
