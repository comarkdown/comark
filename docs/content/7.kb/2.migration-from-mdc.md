...
- **Binding**: `{{ path }}` via opt-in [binding plugin](/plugins/built-in/binding) + render-time `data` prop (namespaced paths)
...
MDC built-in bindings map to the opt-in [binding plugin](/plugins/built-in/binding). See [Binding and data passing](#binding-and-data-passing) for the full before/after (namespaced paths, render-time `data`).

```typescript
import { parseMarkdown } from 'comark'
import binding from 'comark/plugins/binding'

const document = await parseMarkdown(md, { plugins: [binding()] })
// Render with Binding + data — do not pass data here
```
...
| `data` (frontmatter) | — | Frontmatter lives on `document.frontmatter`, not a separate prop |
| — | `data` | **New meaning**: runtime values for [data binding](/syntax/components#data-binding) / [`{{ }}`](/plugins/built-in/binding) — not frontmatter |
...
MDC's `{{ }}` interpolation and runtime data are supported in Comark via the [binding plugin](/plugins/built-in/binding) and the shared [data-binding](/syntax/components#data-binding) layer. The API differs in three important ways:

1. **Opt-in parser + renderer handler** — register `binding()` and the framework `Binding` component (not enabled by default).
2. **Namespaced paths** — use `frontmatter.`, `data.`, `meta.`, or `props.` instead of bare names from a flat parse-time bag.
3. **Data at render time** — pass runtime values with the renderer's `data` prop. Do **not** pass `{ data }` into `parseMarkdown`.

### Inline `{{ }}` interpolation

::code-group
```mdc [Before]
<!-- MDC: bare paths from parse-time `data` -->
Hello {{ user.name }}, you have {{ count }} messages.
```
```mdc [After]
<!-- Comark: qualified paths against the render context -->
Hello {{ data.user.name }}, you have {{ data.count }} messages.
...
Wire the plugin once (Nuxt example):

```vue
<script setup lang="ts">
import binding, { Binding } from '@comark/nuxt/plugins/binding'

const markdown = 'Hello {{ data.user.name }}, you have {{ data.count }} messages.'
</script>
...
::code-group
```typescript [Before]
// MDC: data merged at parse time
const result = await parseMarkdown(md, { data: { user, count } })
```
```typescript [After]
// Comark: parse without data, pass data when rendering
const document = await parseMarkdown(md, { plugins: [binding()] })

// Vue / Nuxt
// <MarkdownDocument :value="document" :data="{ user, count }" :components="{ Binding }" />
```
::

`:prefixed` component props (`:label="data.user.name"`, `:answer="frontmatter.theAnswer"`) resolve against the same context **without** the binding plugin. Use the plugin only when you need inline `{{ path }}` text interpolation (or `::if` with the renderer `If` component).

::tip{to="/plugins/built-in/binding"}
Full reference: binding plugin, `||` defaults, `::if` / `#else`, and per-renderer imports.
::
...
| Binding `{{ var }}` | Built-in, bare paths + parse-time `data` | `binding()` + `Binding`; paths like `data.x` / `frontmatter.x`; render-time `:data` |
| Prop data binding | Parse-time `data` + `{{ }}` | `:prop="path"` always; `data` prop on renderer |
