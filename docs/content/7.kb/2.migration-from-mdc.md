...
const tree = await parseMarkdown(md, { plugins: [toc({ depth: 3 })] })
...
const tree = await parseMarkdown(md, { plugins: [summary()] })
const summaryNodes = tree.meta.summary  // Node[] | undefined
```
::
...
const tree = await parseMarkdown(md, { plugins: [emoji()] })
```
::
...
```
```vue [After]
<MarkdownDocument :value="tree" :components="components" />
```
::
...
For a pre-parsed tree, use `<MarkdownDocument>` directly instead of passing it to the all-in-one component.
