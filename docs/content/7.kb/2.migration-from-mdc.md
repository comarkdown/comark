...
`renderMarkdown` includes frontmatter automatically: it reads `document.frontmatter` and serializes it as YAML front matter. See the [Render API](/api/render) for full options.
...
<template>
  <ArticleRenderer :value="document" />
</template>
```
