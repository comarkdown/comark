...
<template>
  <div>
    ...
    <div v-else-if="!state.isComplete" class="loading">
      <div class="spinner" />
      <div>Loading {{ state.body.children.length }} elements...</div>
    </div>
...
<template>
  <div>
    ...
    <MDCRenderer v-if="state.body.children.length > 0" :body="state.body" />
  </div>
</template>
```
...
```typescript
import type { MDCRoot, MDCStreamOptions } from 'mdc-syntax/vue'
import { useMDCStream } from 'mdc-syntax/vue'
...
const options: MDCStreamOptions = {
  ...
  onComplete(result: { body: MDCRoot, data: any, toc?: any }) {
