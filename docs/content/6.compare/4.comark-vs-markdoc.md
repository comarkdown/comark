...
Markdoc nodes are objects carrying `type`, `tag`, `attributes`, `children`, and transform metadata. Comark's format is designed to be sent over the wire: parse on the server, `JSON.stringify` the result, and hydrate any renderer with it. See the [AST reference](/syntax/markdown-ast).
