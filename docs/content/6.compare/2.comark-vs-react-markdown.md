...
Comark treats incomplete input as a first-class case. [Auto-close](/api/auto-close) completes unterminated syntax at every frame, and the `streaming` prop handles it end to end:
...
const tree = await parseMarkdown(markdown) // cache it, store it, send it
```
...
The AST is plain arrays like `['h1', { id: 'hello' }, 'Hello']`, cheap to serialize and cache. See the [AST reference](/syntax/markdown-ast).
