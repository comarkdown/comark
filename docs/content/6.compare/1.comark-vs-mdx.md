...
Because `parseMarkdown(markdown)` returns a serializable [`MarkdownDocument`](/getting-started/document-model), the full CMS pattern works without a build pipeline:

1. Store raw Markdown (components included) in a database or CMS
2. Call `parseMarkdown(markdown)` on request, on the server or in the browser
3. Send the compact AST to the client
4. Render it with a components map: `<MarkdownDocument :value="ast" :components="{ Alert }" />`
...
Comark's [auto-close](/reference/auto-close) parser closes incomplete syntax (unterminated bold, unfinished code fences, half-open components) so every frame renders as valid UI:
...
::accordion
  ...
  Yes. `parseMarkdown()` runs anywhere JavaScript runs. You can parse at build time and ship the AST, parse on the server per request, or parse in the browser. Same API, your choice.
  :::
