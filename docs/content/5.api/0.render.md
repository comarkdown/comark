...
| --- | --- | --- | --- |
| [`maxInlineAttributes`](#code-maxinlineattributes) | `number` | `3` | Maximum attributes before switching to YAML block syntax. Set to `0` to always use block syntax |
...
| [`frontmatterOptions`](#code-frontmatteroptions) | `DumpOptions` | `{ indent: 2, lineWidth: -1 }` | js-yaml [DumpOptions](https://github.com/nodeca/js-yaml#dump-object---options-) passed to the frontmatter serializer |
| [`components`](#code-components) | `Record<string, NodeHandler \| ConditionalNodeHandler>` | `{}` | Custom render handlers for specific elements |
| [`data`](#code-data) | `Record<string, any>` | `{}` | Additional data passed to render handlers |
...
Some plugins export pre-built conditional handlers. For example, the [footnotes plugin](/plugins/core/footnotes#stringify-markdown-rendering) provides `footnotesStringify`:
