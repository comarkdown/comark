...
1. **Use `createParse()` for multiple documents**: When parsing more than one document, always use `createParse()` instead of `parse()` to avoid recreating the parser and plugins on every call.
...
const parse = createParse()
const results = await Promise.all(
  files.map(file => parse(file.content))
)
```
