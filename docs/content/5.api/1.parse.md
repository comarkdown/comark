...
| `registerDefaultPlugins` | `boolean` | `true` | Register the built-in default plugins (`alert`, `task-list`, `syntax`). Set `false` to disable. Also can be used to configure default plugins like `syntax` in conjunction with `plugins` |
