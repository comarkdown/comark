...
### `createParser(options?): (source: string) => Promise<ComarkTree>`{lang="ts"}

Creates a reusable parser function with pre-configured options. Unlike `parse()` which creates a new parser instance on each call, `createParser()` returns a parser function that can be called multiple times with the same configuration.
...
const doc1 = await parser('# Document 1\n\nContent...')
const doc2 = await parser('# Document 2\n\nMore content...')
const doc3 = await parser('# Document 3\n\nEven more...')
```
...
async function buildSite(contentDir: string, outDir: string) {
  ...
  // Parse all files with the same parser instance
  for (const file of mdFiles) {
    const content = await readFile(join(contentDir, file), 'utf-8')
    const tree = await parser(content)
...
app.post('/api/markdown', async (req, res) => {
  try {
    const tree = await parser(req.body.content)
...
```typescript
import { parse, createParser } from 'comark'
import highlight from 'comark/plugins/highlight'
...
console.time('createParser x1000')
const parser = createParser({
  plugins: [highlight()]
})
for (let i = 0; i < 1000; i++) {
...
console.timeEnd('createParser x1000')
...
For better performance when processing multiple files, use `createParser()` instead of `parse()`:
...
1. **Use `createParser()` for multiple documents**: When parsing more than one document, always use `createParser()` instead of `parse()` to avoid recreating the parser and plugins on every call.

```typescript
// ❌ Slow: Creates new parser for each file
for (const file of files) {
  await parse(file.content)
}

// ✅ Fast: Reuses same parser instance
const parser = createParser()
for (const file of files) {
  await parser(file.content)
}
```

2. **Batch process files in parallel**: Use `Promise.all()` to parse multiple files concurrently.

```typescript
const parser = createParser()
const results = await Promise.all(
  files.map(file => parser(file.content))
)
```

3. **Cache parsed results**: If parsing the same content multiple times, cache the `ComarkTree` to avoid redundant parsing.
