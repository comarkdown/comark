---
title: Parse API
description: Parse Markdown with components into a compact Markdown AST using parse() and createParse(), on the server, in the browser, or from a stream.
links:
  - label: Markdown AST
...
**Returns:** `MarkdownTree` object containing:
...
::tip
For full type definitions, see [ComarkNode](/syntax/markdown-ast#comarknode) in the Markdown AST reference.
::
...
**Returns:** An async parser function `(source: string) => Promise<MarkdownTree>`
