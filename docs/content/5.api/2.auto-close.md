...
```bash
npm install ai @ai-sdk/gateway @ai-sdk/vue zod
```

#### Create a server route

For this example, you can use two tools so the model learns Comark syntax and your available components before responding:
...
::tip
`fetchComponents` returns a hardcoded string here for simplicity. In a real app it could fetch a JSON schema of your design system, read component source files to extract props and slots, call a registry API, or statically analyse your `components/` directory.
::
...
See the full working example — server route, Alert component, and chat UI wired together.
::
