...
description: The auto-close API provides utilities for handling incomplete or partial Comark content, which is especially useful for streaming scenarios where content arrives incrementally.
---

## Overview

When parsing streaming content (like AI-generated text), the content is often incomplete with unclosed markdown syntax or Comark components. The auto-close utilities automatically detect and close these unclosed elements, ensuring valid parsing at every chunk.

## Functions

### `autoCloseMarkdown(markdown: string): string`

Automatically closes unclosed markdown inline syntax and Comark components.
...
The auto-close utilities are built into the parser and enabled by default:

```typescript
import { parse } from 'comark'

// Auto-close is enabled by default
...
// Disable auto-close if needed
const resultWithoutAutoClose = await parse(content, {
  autoClose: false
})
```
...
// Manual auto-close before parsing
const partialContent = '**bold text'
const closed = autoCloseMarkdown(partialContent)
const parsed = await parse(closed)
...
socket.on('end', () => {
  ...
  const final = await parse(accumulated)
...
2. **Custom syntax**: Only standard Comark and markdown syntax is supported. Custom extensions need separate handling.
