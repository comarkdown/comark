...
`renderAnsi()` accepts all [`ParserOptions`](/api/parse#options) in addition to the ANSI renderer options above.
...
const writeAnsi = createAnsiWriter({ plugins: [highlight()] })
