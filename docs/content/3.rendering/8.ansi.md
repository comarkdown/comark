...
The `@comark/ansi` package renders Markdown AST to ANSI-styled strings for terminal output. Install it separately:
...
Creates a reusable log function with pre-configured options. The underlying parser is initialized once and reused on every call, which is more efficient when logging many documents.
...
Render a pre-parsed Comark tree to an ANSI string, with no parsing step. Use this when you already have a parsed tree and want to avoid re-parsing.
...
type ComponentRenderer = (element: MarkdownElement, ctx: RenderContext) => Promise<string>
