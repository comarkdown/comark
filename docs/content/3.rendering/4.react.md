...
The `extends` option lets you build on top of an existing defined component. The child inherits the parent's plugins and components, then layers its own on top.
...
```tsx [App.tsx]
import { ArticleComark, CommentComark } from './comark'

export default function Page({ article, comments }) {
  return (
    <>
      <ArticleComark>{article.body}</ArticleComark>
      {comments.map(c => (
        <CommentComark key={c.id}>{c.body}</CommentComark>
      ))}
    </>
  )
}
...
// Base renderer with shared element overrides
...
// Comment renderer — lighter alert variant, inherits CodeBlock and ProseA
...
| `components` | `Record<string, ComponentType>` | `{}` | Additional component overrides (merged on top of config) |
