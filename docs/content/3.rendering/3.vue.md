...
A `path` is a node-index path into `tree.nodes`: the first segment indexes the top-level nodes, each later segment indexes into that element's children. Patch operations are `replace`, `insert`, `remove` (each takes a `path`), plus `meta` and `frontmatter` merges. Websocket handlers, agents, devtools, and HMR all drive the context the same way.
