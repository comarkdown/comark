...
::tip{to="/rendering/vue#code-markdownparsed"}
For server-side rendering, use the `MarkdownParsed` component combined with parsing on the server instead.
::
...
Pass markdown content via the default slot or the `value` prop. `value` accepts a markdown **string** or a pre-parsed [`MarkdownTree`](/syntax/markdown-ast):
...
|------|------|---------|-------------|
| `value` | `string \| MarkdownTree` | `undefined` | Markdown string or pre-parsed tree (alternative to default slot) |
...
For lazy-loading components on demand. Components are resolved once and cached. Works with both `<Markdown>` and `<MarkdownParsed>`:
...
export const BaseComark = defineMarkdownComponent({
  name: 'BaseComark',
  plugins: [
...
export const CommentComark = defineMarkdownComponent({
  name: 'CommentComark',
  extends: BaseComark,
})
```
...
Renders a pre-parsed `MarkdownTree` without any parsing. Use it when you parse on the server, in a build step, or via an API, so no parser or plugin code is shipped to the browser.
...
Creates a pre-configured `<MarkdownParsed>` with baked-in component mappings.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| [`extends`](#code-markdownparsed-code-definemarkdownparsedcomponent-inheritance) | `ReturnType<typeof defineMarkdownParsedComponent>` | `undefined` | Inherit component mappings from another renderer |
| `name` | `string` | `undefined` | Component name for debugging |
| [`components`](#code-markdown-props-code-components) | `Record<string, Component>` | `{}` | Custom Vue component mappings |
| `class` | `string` | `undefined` | Additional CSS classes for the wrapper div |
...
const BaseRenderer = defineMarkdownParsedComponent({
  name: 'BaseRenderer',
...
export const CommentRenderer = defineMarkdownParsedComponent({
...
`MarkdownParsed` can subscribe to an ambient **context** so external sources can drive a mounted renderer: an HMR signal, a collaboration socket, an agent editing the document while you chat with it, or devtools. The listen key is the tree's own `meta.key` (set by a plugin) or the `comark-key` prop, so a parsed document can carry its own identity. If `globalThis.comarkContext` exists the renderer listens for updates on that key and re-renders; on unmount it cleans up. With no context, the key is ignored at zero cost.

```vue
<template>
  <MarkdownParsed comark-key="page" :value="tree" />
</template>
```
...
Override how native HTML elements render by mapping a component to their tag name via the `components` prop in both the `Markdown` and `MarkdownParsed` components.
...
Use `ComarkPlugin` from `comark` to type plugin arrays, and `MarkdownElement` to type the `__node` prop in components that override HTML elements:
