...
Pass markdown content via the default slot or the `value` prop. `value` accepts a markdown **string** or a pre-parsed [`MarkdownDocument`](/syntax/markdown-ast):
...
defineProps<{ tree: MarkdownDocument }>()
</script>
...
|------|------|---------|-------------|
| `value` | `string \| MarkdownDocument` | `undefined` | Markdown string or pre-parsed tree (alternative to default slot) |
...
<template>
  <MarkdownDocument :value="tree" :components="{ alert: Alert }" />
</template>
```
