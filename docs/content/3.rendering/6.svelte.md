...
<Markdown value={content} data={{ name: 'Ada' }} plugins={[binding()]} components={{ Binding, If }} />
```
...
<script>
  ...
  let name = $state(model.get('data.name'))
  model.subscribe('data.name', (v) => { name = v })
</script>

<p>Current name: {name}</p>
<Markdown value=":input{::value=\"data.name\" type=\"text\"}" {model} plugins={[binding()]} components={{ Binding, If }} />
```
