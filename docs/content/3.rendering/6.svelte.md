...
**Lazy loading with SvelteKit SSR** — use `<ComarkAsync>` and a non-eager `import.meta.glob`. SvelteKit awaits async SSR work, so only rendered components are loaded and included in the server HTML:
...
<script lang="ts">
  ...
  const componentsManifest = (name: string) => {
    return modules[`../lib/components/comark/${pascalCase}.svelte`]?.()
  }
</script>
...
::tip
`import.meta.glob` resolves matching files at **build time**. With `eager: true`, modules are imported statically. Without it, Vite generates dynamic `import()` calls that load each module lazily at runtime.
::
