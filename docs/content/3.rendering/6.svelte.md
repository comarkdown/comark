...
**Lazy loading** — components are imported on demand when first rendered:
...
**Eager loading for SSR** — use `import.meta.glob` with `eager: true` so the server can render components synchronously (no `await` needed):
...
<script lang="ts">
  ...
  const componentsManifest = (name: string) => {
    return modules[`./components/${name}.svelte`]
  }
</script>
...
::tip
`import.meta.glob` resolves matching files at **build time**. With `eager: true`, modules are imported statically — ideal for SSR. Without it, Vite generates dynamic `import()` calls that load each module lazily at runtime.
::
...
```svelte [components/prose/ProseH1.svelte]
<script lang="ts">
  import type { Snippet } from 'svelte'
