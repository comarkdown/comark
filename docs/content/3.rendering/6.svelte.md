...
Renders a pre-parsed `ComarkTree` without any parsing. Use it when you parse on the server (e.g., in a SvelteKit `load` function), so no parser or plugin code is shipped to the browser.
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `value` | `ComarkTree` | — | **Required.** The parsed tree returned by `parse()` |
| [`components`](#code-markdown-props-code-components) | `Record<string, Component>` | `{}` | Custom Svelte component mappings |
