...
Pass markdown content via the `value` prop. `value` accepts a markdown **string** or a pre-parsed [`MarkdownDocument`](/syntax/markdown-ast):
...
<script lang="ts">
  ...
  let { tree }: { tree: MarkdownDocument } = $props()
</script>
...
|------|------|---------|-------------|
| `value` | `string \| MarkdownDocument` | `''` | Markdown string or pre-parsed tree |
...
<MarkdownDocument value={data.tree} {componentsManifest} />
```
...
<MarkdownDocument value={data.tree} componentsManifest={manifest} />
```
