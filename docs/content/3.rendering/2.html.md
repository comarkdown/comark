...
Use a conditional handler when matching should depend on attributes, not just the tag name:
...
const render = createRender({
  components: {
    ...
    infoAlert: {
      ...
      handler: async ([, attrs, ...children], { render }) => {
...
const render = createRender({
  data,
  components: {
    header: async ([, attrs, ...children], { render, data }) => {
...
| --- | --- | --- |
| `components` | `Record<string, NodeHandler \| ConditionalNodeHandler>` | Custom component renderers |
