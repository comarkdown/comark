...
| `html` | `boolean` | `true` | Parse embedded HTML into document nodes |
...
`renderHtml()` accepts all [`ParserOptions`](/api/parse#options) in addition to the renderer options above.
...
const renderHtml = createHtmlRenderer({ plugins: [highlight()] })
