...
Use `defineComarkComponent` and `defineComarkRendererComponent` to create pre-configured wrappers with default plugins, components, and styling baked in, so there is no need to repeat the same config on every instance.
