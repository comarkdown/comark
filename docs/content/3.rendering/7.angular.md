...
Use `::prop="data.path"` in your markdown to wire native form elements or pass live values to custom components.

### Uncontrolled (default)

When no `model` input is provided, `<comark-markdown>` and `<comark-markdown-document>` create an internal `createModelStore()` seeded from `data`. Writes stay local to the component.
...
model = createModelStore({ data: { data: { name: 'Ada' } } })
```
...
Native form events are wired via `Renderer2.listen` so no Zone.js issues arise.

| Markdown | Element | Wired prop | Event |
|----------|---------|------------|-------|
| `:input{::value="…" type="text"}` | `<input>` | `value` | `input` |
| `:input{::checked="…" type="checkbox"}` | `<input>` | `checked` | `change` |
| `:input{::value="…" type="number"}` | `<input>` | `value` (as number) | `input` |
| `:select{::value="…"}` | `<select>` | `value` | `change` |
