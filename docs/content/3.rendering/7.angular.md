...
The `<comark-markdown>` component is the simplest way to render markdown in Angular. It handles parsing and rendering automatically. The `value` input accepts a markdown **string** or a pre-parsed [`MarkdownDocument`](/syntax/markdown-ast).
...
Passing a tree to `<comark-markdown>` skips parsing at runtime, but the **parser is still bundled** because `Markdown` imports it. To keep the client bundle free of the parser, use [`<comark-markdown-document>`](#code-comark-markdown-document) instead.
::
...
@Component({
  ...
  template: `
    @if (tree) {
      <comark-markdown-document [value]="tree" [components]="components" />
    }
  `,
})
export class DocsComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.tree = await parseMarkdown(markdown)
  }
}
```
