...
export default function App() {
  const name = useSyncExternalStore(
    (cb) => model.subscribe('data.name', cb),
    () => model.get('data.name') as string
  )
  return (
    <>
...
`::value`, `::checked`, and `::files` on native `<input>`, `<select>`, and `<textarea>` are wired automatically. React events (`onChange`, `onInput`) and controlled-element props are set for you.
...
<Markdown
  value={content}
  onModelChange={(path, value, snapshot) => {
    console.log(path, value, snapshot)
  }}
/>
```
