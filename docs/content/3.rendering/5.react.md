...
`MarkdownDocument` can subscribe to an ambient **context** so external sources can drive a mounted renderer: an HMR signal, a collaboration socket, an agent editing the document while you chat with it, or devtools. Pass a `comarkKey`, and if `globalThis.comarkContext` exists the renderer listens for updates on that key and re-renders; on unmount it cleans up. The key falls back to the document's own `meta.key` when a plugin sets it. With no context, it's a no-op at zero cost.
