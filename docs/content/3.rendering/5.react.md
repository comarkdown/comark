...
Pass markdown content via `children` or the `value` prop. `value` accepts a markdown **string** or a pre-parsed [`MarkdownDocument`](/syntax/markdown-ast):
...
export default function Article({ tree }: { tree: MarkdownDocument }) {
...
Passing a tree to `<Markdown>` skips parsing at runtime, but the **parser is still bundled** because `Markdown` imports it. To keep the client bundle free of the parser, use [`<MarkdownDocument>`](#code-markdowndocument) instead.
::
...
| `value` | `string \| MarkdownDocument` | `''` | Markdown string or pre-parsed tree (alternative to children) |
...
export default async function DocsPage({ params }: { params: { slug: string } }) {
  ...
  return <MarkdownDocument value={tree} components={{ alert: Alert }} />
}
```
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `value` | `MarkdownDocument` | — | **Required.** The parsed tree returned by `parse()` |
| [`components`](#code-markdown-props-code-components) | `Record<string, ComponentType>` | `{}` | Custom React component mappings |
...
const BaseRenderer = defineMarkdownDocumentComponent({
  name: 'BaseRenderer',
...
export const CommentRenderer = defineMarkdownDocumentComponent({
...
```tsx
<MarkdownDocument comarkKey="page" value={tree} />
```
