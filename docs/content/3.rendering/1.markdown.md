...
Override how specific elements are serialized back to markdown by passing `components`:
...
Each handler receives the `ComarkElement` tuple `[tag, attributes, ...children]` and a context with:
- `render(children)` — renders child nodes
- `data` — additional data passed via `options.data`
