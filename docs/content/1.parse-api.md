...
for await (const result of parseStreamIncremental(stream)) {
  if (!result.isComplete) {
    ...
    console.log(`Parsed ${result.body.children.length} elements so far`)
  }
}
```
...
// MDCRoot with only the content before <!-- more -->
```
...
- [Auto-Close API](./auto-close-api) - Handling incomplete syntax
- [MDCRenderer Component](./mdc-renderer) - Rendering parsed content in Vue
