...
Plugins are always exported as factory functions, which allows them to accept options:

```typescript
export default function myPlugin(options = {}): ComarkPlugin {
  return {
    name: 'my-plugin',
    // ...
  }
...
export default function removeComments(): ComarkPlugin {
  return {
...
export default function wordCount(): ComarkPlugin {
  return {
    name: 'word-count',
    post(state) {
      let count = 0
...
To use an existing markdown-it plugin, wrap it in a `ComarkPlugin`:
...
function subscript(): ComarkPlugin {
  return {
    name: 'subscript',
...
function superscript(): ComarkPlugin {
  return {
    name: 'superscript',
...
export default function highlight(): ComarkPlugin {
  return {
    name: 'highlight',
...
export default function autoLink(): ComarkPlugin {
  return {
    name: 'auto-link',
    post(state) {
...
export default function styledTables(): ComarkPlugin {
  return {
    name: 'styled-tables',
    post(state) {
      visit(state.tree,
