...
The plugin exports a `footnotesStringify` conditional handler that converts the footnote AST back into standard markdown footnote syntax. This is useful when you want to render a `ComarkTree` back to markdown and preserve footnotes.
...
Without `footnotesStringify`, `renderMarkdown` would serialize the footnote HTML structure (sup, section, ol, etc.) as Comark component syntax instead of native footnote syntax.
...
- [Task List](/plugins/core/task-list) — Checkbox syntax
