...
- `tree` - The [`MarkdownDocument`](/getting-started/document-model#document-structure) to traverse
- `checker` - A predicate `(node: Node) => boolean`, return `true` to visit a node
- `visitor` - A transform `(node: Node) => Node | false | void`, see [`Node`](/getting-started/document-model#node-model)
