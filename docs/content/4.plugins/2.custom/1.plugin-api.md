...
| `tree` | [`MarkdownTree<TMeta, TFrontmatter>`](/syntax/markdown-ast#markdowntree) | The parsed AST; modify to transform output. `tree.meta` / `tree.frontmatter` are typed against the plugin's declared contribution. |
| `options` | [`ParseOptions`](/api/parse#options) | The parser configuration |
