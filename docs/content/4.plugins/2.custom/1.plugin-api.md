...
| `tree` | [`MarkdownDocument<TMeta, TFrontmatter>`](/syntax/markdown-ast#markdowndocument) | The parsed AST; modify to transform output. `tree.meta` / `tree.frontmatter` are typed against the plugin's declared contribution. |
| `options` | [`ParserOptions`](/api/parse#options) | The parser configuration |
