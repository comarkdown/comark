...
| `options` | [`ParserOptions`](/reference/parse#options) | The parser configuration |
...
| `tree` | [`MarkdownDocument<TMeta, TFrontmatter>`](/getting-started/document-model#document-structure) | The parsed document; modify it to transform output. `tree.meta` / `tree.frontmatter` are typed against the plugin's declared contribution. |
| `options` | [`ParserOptions`](/reference/parse#options) | The parser configuration |
