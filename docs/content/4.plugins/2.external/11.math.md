...
The Math plugin enables rendering of LaTeX math formulas in your markdown content using [KaTeX](https://katex.org/). It supports both inline and display math expressions.

The plugin is included in `@comark/vue` and `@comark/react`. You only need to install the `katex` peer dependency:
