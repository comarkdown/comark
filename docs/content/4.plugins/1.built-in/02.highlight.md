...
Comark detects the language from the code fence info string (e.g. ` ```javascript `) and highlights it using languages registered in the highlight plugin. Register languages via the `languages` option or rely on the default set when `defaultLanguages` is `true`.
...
When you use code block line highlight syntax (ex. `{1,3-5}`), lines receive a `.highlight` class automatically. Add the following styles to give highlighted lines a visible background:
