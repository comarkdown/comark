...
Controls whether to register a set of commonly-used languages. When `true` (default), these languages are automatically registered: `vue`, `typescript`, `javascript`, `mdc`, `tsx`, `bash`, `json`, `yaml`. For any other languages, you need to provide them via the `languages` option.
