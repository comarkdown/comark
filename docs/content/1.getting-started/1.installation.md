...
|---|---|---|---|
| **Package** | `@comark/vue` `@comark/react` `@comark/nuxt` `@comark/svelte` `@comark/angular` | `@comark/html` `@comark/ansi` | `comark` |
| **Provides** | `<Markdown>`, `<MarkdownDocument>` (internal parsing and rendering components) | `renderANSI()`, `renderHTML()` (not framework related) | `parseMarkdown()`, `renderMarkdown()` (programmatic parsing and rendering) |
...
::tabs{class="gap-0"}
  ...
  ::tab-item{label="Svelte" icon="i-logos-svelte-icon"}
    ...
    ::
  ::
  ::tab-item{label="Angular" icon="i-simple-icons-angular"}
    ::code-group
    ```bash [pnpm]
...
::card-group{cols="2"}
  ...
  ::card{icon="i-simple-icons-angular" title="Angular" to="/rendering/angular"}
