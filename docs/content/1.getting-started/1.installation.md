...
| **Provides** | `<Comark>`, `<ComarkRenderer>` — internal parsing and rendering components | `render()`, `renderHTML()` — not framework related | `parse()`, `renderMarkdown()` — programmatic parsing and rendering |
...
::tabs{class="gap-0"}
  ...
  ::tab-item{label="React" icon="i-logos-react"}
    ...
    ::
  ::
  ::tab-item{label="Nuxt" icon="i-simple-icons-nuxt"}
    ::code-group
    ```bash [pnpm]
    ...
    ```
  ::
  ::tab-item{label="Svelte" icon="i-simple-icons-svelte"}
    ::code-group
    ```bash [pnpm]
...
::card-group{cols="2"}
  ...
  ::card{icon="i-simple-icons-nuxt" title="Nuxt" to="/rendering/nuxt"}
  Zero-config Nuxt module with auto-imported components and Nuxt UI integration.
  ::

  ::card{icon="i-simple-icons-svelte" title="Svelte" to="/rendering/svelte"}
