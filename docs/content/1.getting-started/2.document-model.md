...
## Deprecated Tree Names

Earlier Comark releases called the parsed document a tree. The `ComarkTree` type and `tree` renderer props remain as deprecated compatibility aliases, but new code should use `MarkdownDocument` and the `value` prop:
