...
- Props authored with the `:` prefix participate in data binding. Plain `prop="value"` attributes are passed as literal strings, except the boolean values `"true"` and `"false"`, which are promoted to typed boolean bindings. Other values, including numeric strings such as `"5"`, remain strings.
