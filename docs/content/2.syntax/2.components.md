...
Bindings also work inside object and array props, whether they come from a YAML props block or inline JSON. Prefix the nested key with `:`:
...
::page-hero{:links='[{"label":"Sign in",":to":"data.platform.dashboard_login_url"}]'}
::
```

Both resolve to `links: [{ label: '…', to: '<value of data.platform.dashboard_login_url>' }]`.
...
- Only props authored with the `:` prefix participate in data binding, at any depth. Plain `prop="value"` attributes (and un-prefixed nested keys) are always passed as literal values.
