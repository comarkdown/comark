...
Original main-branch aliases remain available as deprecated exports: `ComarkText`, `ComarkElement`, `ComarkElementAttributes`, `ComarkComment`, `ComarkNode`, and `ComarkTree`.
::
...
```typescript [find.ts]
function findByTag(tree: MarkdownDocument, tag: string): Node[] {
  const results: Node[] = []
...
function addClassToLinks(node: Node): Node {
  if (Array.isArray(node)) {
...
// Transform all links
const transformed: MarkdownDocument = {
...
import type { Node, ElementNode, TextNode } from 'comark'
