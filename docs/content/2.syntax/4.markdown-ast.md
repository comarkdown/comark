---
title: Markdown AST
description: Complete reference for the Markdown AST (Abstract Syntax Tree) format, a lightweight array-based structure for efficient processing.
navigation:
  title: AST
links:
  - label: Components
    icon: i-lucide-component
    to: /syntax/components
    color: neutral
    variant: soft
  - label: Markdown
    icon: i-lucide-file-text
    to: /syntax/markdown
    color: neutral
    variant: soft
---

## Overview

**Markdown AST** is a lightweight, array-based AST format designed for efficient processing and minimal memory usage. Unlike traditional node-based ASTs, Comark uses nested arrays (tuples) to represent document structure.

## Structure

### MarkdownTree

The root container for all parsed content:

```typescript
interface MarkdownTree {
  nodes: ComarkNode[]                   // Parsed AST nodes
  frontmatter: Record<string, any>      // YAML frontmatter data
  meta: {
    toc?: any                           // Table of contents (from toc plugin)
    summary?: ComarkNode[]              // Summary content (from summary plugin)
    [key: string]: any                  // Other plugin metadata
  }
}
```

### ComarkNode

Each node is either a text string, an element tuple, or a comment:

```typescript
type ComarkNode = MarkdownElement | MarkdownText | MarkdownComment
```

### MarkdownText

Plain string representing text content:

```typescript
type MarkdownText = string
```

::callout{color="neutral" icon="i-lucide-history"}
`ComarkText` remains exported as a deprecated alias of `MarkdownText`.
::

### MarkdownElement

An element is a tuple array with tag, attributes, and children:

```typescript
type MarkdownElement = [string, MarkdownElementAttributes, ...ComarkNode[]]
```

::callout{color="neutral" icon="i-lucide-history"}
`ComarkElement` remains exported as a deprecated alias of `MarkdownElement`.
::

### MarkdownElementAttributes

Attributes are a key-value record (plus optional internal `$` meta):

```typescript
type MarkdownElementAttributes = {
  [key: string]: unknown
  $?: {
    line?: number
    html?: 0 | 1
    block?: 0 | 1
  }
}
```

::callout{color="neutral" icon="i-lucide-history"}
`ComarkElementAttributes` remains exported as a deprecated alias of `MarkdownElementAttributes`.
::

### MarkdownComment

HTML comments are represented as a tuple with `null` as the tag:

```typescript
type MarkdownComment = [null, MarkdownElementAttributes, string]
```

::callout{color="neutral" icon="i-lucide-history"}
`ComarkComment` remains exported as a deprecated alias of `MarkdownComment`.
::

## Node Format

### Text Nodes

Plain strings represent text content:

```json
"Hello, World!"
```

### Element Nodes

Elements are arrays with the format `[tag, props, ...children]`:

```json
["p", {}, "This is a paragraph"]
```

**Components:**
- **tag** (index 0): Element name or component tag
- **props** (index 1): Object with attributes/properties
- **children** (index 2+): Child nodes (strings or nested arrays)

## Markdown to AST

### Standards

#### Headings

::code-group

```mdc [Syntax]
# Heading 1
## Heading 2
```

```json [AST]
["h1", { "id": "heading-1" }, "Heading 1"],
["h2", { "id": "heading-2" }, "Heading 2"]
```

::

#### Paragraphs

::code-group

```mdc [Syntax]
This is a paragraph with **bold** and *italic* text.
```

```json [AST]
["p", {},
  "This is a paragraph with ",
  ["strong", {}, "bold"],
  " and ",
  ["em", {}, "italic"],
  " text."
]
```

::

#### Links

::code-group

```mdc [Syntax]
[Link text](https://example.com)
```

```json [AST]
["a", { "href": "https://example.com" }, "Link text"]
```

::

#### Images

::code-group

```mdc [Syntax]
![Alt text](image.png)
```

```json [AST]
["img", { "src": "image.png", "alt": "Alt text" }]
```

::

#### Lists

::code-group

```mdc [Syntax]
- Item 1
- Item 2
  - Nested item
```

```json [AST]
["ul", {},
  ["li", {}, "Item 1"],
  ["li", {},
    "Item 2",
    ["ul", {},
      ["li", {}, "Nested item"]
    ]
  ]
]
```

::

#### Ordered Lists

::code-group

```mdc [Syntax]
1. First
2. Second
```

```json [AST]
["ol", {},
  ["li", {}, "First"],
  ["li", {}, "Second"]
]
```

::

#### Code Blocks

::code-group

````mdc [Syntax]
```javascript [app.js] {1-2}
const a = 1
const b = 2
```
````

```json [AST]
["pre", {
    "language": "javascript",
    "filename": "app.js",
    "highlights": [1, 2]
  },
  ["code", { "class": "language-javascript" },
    "const a = 1\nconst b = 2"
  ]
]
```

::

#### Inline Code

::code-group

```mdc [Syntax]
Use `const` for constants
```

```json [AST]
["p", {},
  "Use ",
  ["code", {}, "const"],
  " for constants"
]
```

::

#### Span Attributes

Span syntax wraps inline text with custom attributes:

::code-group

```mdc [Syntax]
This is [highlighted]{.highlight .yellow} text.
```

```json [AST]
["p", {},
  "This is ",
  ["span", {
    "class": "highlight yellow"
  }, "highlighted"],
  " text."
]
```

::

**With multiple attributes:**

::code-group

```mdc [Syntax]
[Important]{#notice .badge style="color: red" data-priority="high"}
```

```json [AST]
["span", {
  "id": "notice",
  "class": "badge",
  "style": "color: red",
  "data-priority": "high"
}, "Important"]
```

::

**With nested markdown:**

::code-group

```mdc [Syntax]
[**Bold** text]{.emphasized}
```

```json [AST]
["span", {
  "class": "emphasized"
},
  ["strong", {}, "Bold"],
  " text"
]
```

::

#### Blockquotes

::code-group

```mdc [Syntax]
> This is a quote
```

```json [AST]
["blockquote", {},
  ["p", {}, "This is a quote"]
]
```

::

#### Tables

::code-group

```mdc [Syntax]
| Header 1 | Header 2 |
| -------- | -------- |
| Cell 1   | Cell 2   |
```

```json [AST]
["table", {},
  ["thead", {},
    ["tr", {},
      ["th", {}, "Header 1"],
      ["th", {}, "Header 2"]
    ]
  ],
  ["tbody", {},
    ["tr", {},
      ["td", {}, "Cell 1"],
      ["td", {}, "Cell 2"]
    ]
  ]
]
```

::

#### Horizontal Rule

::code-group

```mdc [Syntax]
---
```

```json [AST]
["hr", {}]
```

::

#### Comments

HTML comments are represented with `null` as the tag:

::code-group

```mdc [Syntax]
<!-- This is a comment -->
```

```json [AST]
[null, {}, " This is a comment "]
```

::

**Inline comment:**

::code-group

```mdc [Syntax]
Text before <!-- comment --> text after
```

```json [AST]
["p", {},
  "Text before ",
  [null, {}, " comment "],
  " text after"
]
```

::

**Multi-line comment:**

::code-group

```mdc [Syntax]
<!--
Multi-line
comment text
-->
```

```json [AST]
[null, {}, "\nMulti-line\ncomment text\n"]
```

::

**Comment between elements:**

::code-group

```mdc [Syntax]
## First Section

<!-- Separator -->

## Second Section
```

```json [AST]
[
  ["h2", { "id": "first-section" }, "First Section"],
  [null, {}, " Separator "],
  ["h2", { "id": "second-section" }, "Second Section"]
]
```

::

### Components

#### Block Components

::code-group

```mdc [Syntax]
::alert{type="info"}
This is an alert message
::
```

```json [AST]
["alert", { "type": "info" },
  ["p", {}, "This is an alert message"]
]
```

::

#### Inline Components

::code-group

```mdc [Syntax]
Check out this :badge[New]{color="blue"} feature
```

```json [AST]
["p", {},
  "Check out this ",
  ["badge", { "color": "blue" }, "New"],
  " feature"
]
```

::

#### Components with Slots

::code-group

```mdc [Syntax]
::card
#header
## Card Title

#content
Main content here
::
```

```json [AST]
["card", {},
  ["template", { "name": "header" },
    ["h2", { "id": "card-title" }, "Card Title"]
  ],
  ["template", { "name": "content" },
    ["p", {}, "Main content here"]
  ]
]
```

::

#### Nested Components

::code-group

```mdc [Syntax]
:::outer
::inner
Content
::
:::
```

```json [AST]
["outer", {},
  ["inner", {},
    ["p", {}, "Content"]
  ]
]
```

::

### Props

#### String Properties

::code-group

```mdc [Syntax]
::component{title="Hello"}
```

```json [AST]
["component", { "title": "Hello" }]
```

::

#### Boolean Properties

::code-group

```mdc [Syntax]
::component{disabled}
```

```json [AST]
["component", { ":disabled": "true" }]
```

::

**Note:** Boolean props are prefixed with `:` in the AST.

#### Number Properties

::code-group

```mdc [Syntax]
::component{:count="5"}
```

```json [AST]
["component", { ":count": "5" }]
```

::

#### Object/Array Properties

::code-group

```mdc [Syntax]
::component{:data='{"key": "value"}'}
```

```json [AST]
["component", { ":data": "{\"key\": \"value\"}" }]
```

::

#### ID and Class Properties

::code-group

```mdc [Syntax]
::component{#my-id .class-one .class-two}
```

```json [AST]
["component", {
  "id": "my-id",
  "class": "class-one class-two"
}]
```

::

### Complete Example

:::code-group

```mdc [Syntax]
---
title: Example Document
author: John Doe
---

# Welcome

This is a **sample** document with [links](https://example.com).

::alert{type="info"}
Important notice
::

## Code Example

\`\`\`javascript [demo.js]
console.log("Hello")
\`\`\`
```

```json [AST]
{
  "nodes": [
    ["h1", { "id": "welcome" }, "Welcome"],
    ["p", {},
      "This is a ",
      ["strong", {}, "sample"],
      " document with ",
      ["a", { "href": "https://example.com" }, "links"],
      "."
    ],
    ["alert", { "type": "info" },
      ["p", {}, "Important notice"]
    ],
    ["h2", { "id": "code-example" }, "Code Example"],
    ["pre", { "language": "javascript", "filename": "demo.js" },
      ["code", { "class": "language-javascript" },
        "console.log(\"Hello\")"
      ]
    ]
  ],
  "frontmatter": {
    "title": "Example Document",
    "author": "John Doe"
  },
  "meta": {}
}
```

:::

## Working with AST

### Traverse

```typescript [traverse.ts]
import type { ComarkNode, MarkdownTree } from 'comark'

function traverse(node: ComarkNode, callback: (node: ComarkNode) => void) {
  callback(node)

  if (Array.isArray(node)) {
    const children = node.slice(2)
    for (const child of children) {
      traverse(child, callback)
    }
  }
}

// Usage
const result = await parse(content)
traverse(result.nodes[0], (node) => {
  if (Array.isArray(node)) {
    console.log('Element:', node[0])
  } else {
    console.log('Text:', node)
  }
})
```

### Find

```typescript [find.ts]
function findByTag(tree: MarkdownTree, tag: string): ComarkNode[] {
  const results: ComarkNode[] = []

  function search(node: ComarkNode) {
    if (Array.isArray(node) && node[0] === tag) {
      results.push(node)
    }
    if (Array.isArray(node)) {
      node.slice(2).forEach(search)
    }
  }

  tree.nodes.forEach(search)
  return results
}

// Find all headings
const headings = findByTag(result, 'h1')
```

### Modify

```typescript [transform.ts]
function addClassToLinks(node: ComarkNode): ComarkNode {
  if (Array.isArray(node)) {
    const [tag, props, ...children] = node

    if (tag === 'a') {
      return [tag, { ...props, class: 'external-link' }, ...children]
    }

    return [tag, props, ...children.map(addClassToLinks)]
  }

  return node
}

// Transform all links
const transformed: MarkdownTree = {
  nodes: result.nodes.map(addClassToLinks),
  frontmatter: result.frontmatter,
  meta: result.meta
}
```

### Extract

```typescript [extract.ts]
function extractText(node: ComarkNode): string {
  if (typeof node === 'string') {
    return node
  }

  if (Array.isArray(node)) {
    return node.slice(2).map(extractText).join('')
  }

  return ''
}

// Get all text from a heading
const heading = ['h1', { id: 'hello' }, 'Hello ', ['strong', {}, 'World']]
console.log(extractText(heading)) // "Hello World"
```

### Type Guards

```typescript [guards.ts]
import type { ComarkNode, MarkdownElement, MarkdownText } from 'comark'

function isElement(node: ComarkNode): node is MarkdownElement {
  return Array.isArray(node) && typeof node[0] === 'string'
}

function isText(node: ComarkNode): node is MarkdownText {
  return typeof node === 'string'
}

// Usage
function processNode(node: ComarkNode) {
  if (isText(node)) {
    console.log('Text:', node)
  } else if (isElement(node)) {
    const [tag, props, ...children] = node
    console.log('Element:', tag, props)
  }
}
```

## Performance Considerations

1. **Array-based format**: More memory efficient than object-based AST
2. **Shallow iteration**: Children start at index 2, making iteration predictable
3. **Immutable by convention**: Create new arrays when modifying
4. **Lazy processing**: Process only what you need

