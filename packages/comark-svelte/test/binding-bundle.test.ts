...
it.each(['none', 'inline', 'if', 'for'] as const)(
  ...
  async (mode) => {
    ...
    const renderer = fileURLToPath(new URL('../src/components/MarkdownDocument.svelte', import.meta.url))
      ...
      plugins: [
        svelte(),
        {
          ...
          load(id) {
            ...
            return (
              `export { default as MarkdownDocument } from ${JSON.stringify(renderer)};` +
