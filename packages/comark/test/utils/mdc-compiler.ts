...
export function mdcCompiler(this: any, options: CompilerOptions = {}) {
  ...
  function compileToJSON(node: Root | RootContent, parent?: Root | RootContent): MDCNode | MDCRoot | null {
    ...
    const position: NodePosition | undefined =
    ...
    if (node.type === 'element') {
      ...
      const children = (
        node.tagName === 'template' && node.content?.children.length ? node.content.children : node.children
      )
    ...
    if (node.type === 'text') {
      if (!/^\n+$/.test(node.value || '') || (parent as any)?.properties?.emptyLinePlaceholder) {
        return options.keepPosition
          ? { type: 'text', value: node.value, position }
    ...
    if (options.keepComments && node.type === 'comment') {
      return options.keepPosition
        ? { type: 'comment', value: node.value, position }
        : { type: 'comment', value: node.value }
    }
