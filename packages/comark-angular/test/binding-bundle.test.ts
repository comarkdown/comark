...
it.each(['none', 'inline', 'if', 'for'] as const)(
  ...
  async (mode) => {
    ...
    const renderer = fileURLToPath(new URL('../dist/components/markdown-document.component.js', import.meta.url))
    const binding = fileURLToPath(new URL('../dist/plugins/binding.js', import.meta.url))
    const result = await build({
      ...
      build: {
        ...
        rollupOptions: { external: ['@angular/core', '@angular/common'] },
      },
    })
