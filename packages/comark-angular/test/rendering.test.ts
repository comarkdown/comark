...
import type { MarkdownElement, MarkdownTree } from 'comark'
...
describe('@comark/angular — rendering integration', () => {
  it('parses heading with bold text into correct AST', async () => {
    ...
    const h1 = tree.nodes[0] as MarkdownElement
    expect(h1[0]).toBe('h1')
  ...
  it('parses component with attributes', async () => {
    ...
    const alert = tree.nodes[0] as MarkdownElement
  ...
  it('parses nested components', async () => {
    ...
    expect(tree.nodes.length).toBeGreaterThan(0)
    const outer = tree.nodes[0] as MarkdownElement
  ...
  it('parses links', async () => {
    ...
    const p = tree.nodes[0] as MarkdownElement
  ...
  it('parses horizontal rules', async () => {
    ...
    const hr = tree.nodes[0] as MarkdownElement
    expect(hr[0]).toBe('hr')
  })
...
::`
    ...
    expect(tree.nodes.length).toBeGreaterThan(0)
    const card = tree.nodes[0] as MarkdownElement
    expect(card[0]).toBe('card')
  })

  it('parses tables', async () => {
    const tree = await parse('| A | B |\n|---|---|\n| 1 | 2 |')
    const table = tree.nodes[0] as MarkdownElement
  ...
  it('generates correct tree shape for renderer consumption', async () => {
    ...
    // paragraph
    const p = tree.nodes[1] as MarkdownElement
    expect(p[0]).toBe('p')
  })
})
