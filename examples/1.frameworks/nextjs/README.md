...
This example demonstrates how to use Comark with Next.js App Router and React Server Components. Blog posts are stored as `.md` files with frontmatter, parsed using `parse()` from `comark`, and rendered using `MarkdownParsed` from `@comark/react` with custom components like `Alert` and `FeatureCard`.
...
- **React rendering** — `MarkdownParsed` from `@comark/react` renders the AST using React Server Components, including custom ones like `Alert` and slot-aware components like `FeatureCard`. No client-side JavaScript is shipped for content rendering.
