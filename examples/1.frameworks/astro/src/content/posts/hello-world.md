...
- **Server-rendered** — Comark parses and renders on the server via `parseMarkdown()` and `renderHtmlFromDocument()`
...
const html = await renderHtmlFromDocument(doc, { components: { alert } })
```
