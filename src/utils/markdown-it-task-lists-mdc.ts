 ...
 *
 * This plugin runs before inline parsing to prevent MDC from interpreting
 * task list markers [X] and [ ] as MDC inline span syntax.
 */
...
export default function markdownItTaskListsMdc(md: MarkdownIt, options?: TaskListOptions) {
  ...
  // Run BEFORE inline parsing to prevent MDC from processing task list markers
      ...
      if (token.type === 'inline' && token.content) {
        ...
        if (parentIdx >= 0) {
          ...
          if (match) {
            ...
            // Replace the task marker with a placeholder that won't be processed by MDC
